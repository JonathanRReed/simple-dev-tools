...
export default function EncodeQR() {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center min-h-screen p-8">
      ...
      >
        <h2 className="text-3xl font-bold text-[#a78bfa] text-center drop-shadow">Encoders & QR</h2>
        <p className="text-bodyText text-center">URL and Base64 encode/decode plus a QR generator (PNG/SVG) — all client-side.</p>
        ...
        {tab === "url" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
            <div>
              <h3 className="text-[#a78bfa] font-semibold mb-2">Input</h3>
              <textarea
              ...
              <div className="mt-2 flex gap-2">
                <button className="px-4 py-2 rounded-xl border border-[#a78bfa] text-white bg-[#23243a]/80" onClick={onUrlEncode}>
                  Encode
                </button>
                <button className="px-4 py-2 rounded-xl border border-[#a78bfa] text-white bg-[#23243a]/80" onClick={onUrlDecode}>
                  Decode
                </button>
              </div>
            ...
            <div>
              <h3 className="text-[#a78bfa] font-semibold mb-2">Output</h3>
              <textarea
                className="w-full min-h-[160px] rounded-xl px-4 py-3 bg-[#0f1021] border border-[#a78bfa33] text-gray-200"
                value={urlOutput}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
            <div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-3">
                ...
                <button className="px-4 py-2 rounded-xl border border-[#a78bfa] text-white bg-[#23243a]/80" onClick={onB64Encode}>
                  Encode
                </button>
                <button className="px-4 py-2 rounded-xl border border-[#a78bfa] text-white bg-[#23243a]/80" onClick={onB64Decode}>
                  Decode
                </button>
              </div>
        ...
        {tab === "qr" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
            <div>
              <h3 className="text-[#a78bfa] font-semibold mb-2">Text</h3>
              <textarea
                className="w-full min-h-[120px] rounded-xl px-4 py-3 bg-[#23243a]/70 border border-[#a78bfa66] text-gray-200 focus:outline-none focus:ring-2 focus:ring-[#a78bfa]"
                value={qrText}
                onChange={(e) => setQrText(e.target.value)}
              />
              <div className="mt-3 grid grid-cols-2 gap-3 items-end">
                <div>
                  <label className="text-gray-300 text-sm">Size (px)</label>
                  <input
                    type="number"
                    min={128}
                    max={1024}
                    className="w-full rounded-xl px-3 py-2 bg-[#23243a]/70 border border-[#a78bfa66] text-gray-200"
                    value={qrSize}
                    onChange={(e) => setQrSize(parseInt(e.target.value || "256", 10))}
                  />
                </div>
                <div>
                  <label className="text-gray-300 text-sm">Error Correction</label>
                  <select
                    className="w-full rounded-xl px-3 py-2 bg-[#23243a]/70 border border-[#a78bfa66] text-gray-200"
                    value={qrEcc}
                ...
                <div className="flex gap-2 justify-end">
                  <button className="px-4 py-2 rounded-xl border border-[#a78bfa] text-white bg-[#23243a]/80" onClick={onGenerateQR}>
                    Generate
                  </button>
                  {qrFormat === "png" && qrPngDataUrl && (
                    <button className="px-4 py-2 rounded-xl border border-[#a78bfa] text-white bg-[#23243a]/80" onClick={downloadPng}>
                      Download PNG
                    </button>
                  )}
                  {qrFormat === "svg" && qrSvg && (
                    <button className="px-4 py-2 rounded-xl border border-[#a78bfa] text-white bg-[#23243a]/80" onClick={downloadSvg}>
                      Download SVG
                    </button>
                  )}
                </div>
            ...
            <div className="flex items-center justify-center min-h-[260px] rounded-xl border border-[#a78bfa33] bg-[#0f1021] p-4">
              {qrFormat === "png" && qrPngDataUrl && (
              ...
              {!qrPngDataUrl && !qrSvg && (
                <div className="text-gray-500 text-sm">Generate a QR to preview it here.</div>
              )}
            </div>
          </div>
        )}

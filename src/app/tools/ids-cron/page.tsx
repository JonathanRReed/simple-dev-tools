...
export default function IDsCronTool() {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center min-h-screen p-8">
      ...
      >
        <h2 className="text-3xl font-bold text-[#a78bfa] text-center drop-shadow">IDs & Scheduling</h2>
        <p className="text-bodyText text-center">Generate UUIDv4 and ULID; describe cron expressions with cRonstrue.</p>
        ...
        {tab === "ids" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
            {/* UUID */}
            <div>
              <h3 className="text-[#a78bfa] font-semibold mb-2">UUID v4</h3>
              <div className="flex items-center gap-2">
                <input
                  className="flex-1 rounded-xl px-4 py-2 bg-[#23243a]/70 border border-[#a78bfa66] text-gray-200"
                  value={uuid}
                  onChange={(e) => setUuid(e.target.value)}
                />
                <button
                  className="px-3 py-2 rounded-xl border border-[#a78bfa] text-white bg-[#23243a]/80"
            ...
            {/* ULID */}
            <div>
              <h3 className="text-[#a78bfa] font-semibold mb-2">ULID</h3>
              <div className="flex items-center gap-2">
                <input
                  className="flex-1 rounded-xl px-4 py-2 bg-[#23243a]/70 border border-[#a78bfa66] text-gray-200"
                  value={ulid}
              ...
              <div className="mt-3 rounded-xl border border-[#a78bfa33] p-3 bg-[#23243a]/40 text-sm">
                {ulidInfo.ok ? (
                  <div className="space-y-1 text-gray-200">
                    <div>
                      <span className="text-gray-400">Timestamp part</span>: <span className="font-mono">{ulidInfo.tsPart}</span>
                    </div>
                    <div>
                      <span className="text-gray-400">Random part</span>: <span className="font-mono break-all">{ulidInfo.randPart}</span>
                    </div>
                    <div>
                      <span className="text-gray-400">Epoch ms</span>: <span className="font-mono">{ulidInfo.ms}</span>
                    </div>
                    <div>
                      <span className="text-gray-400">ISO time</span>: <span className="font-mono">{ulidInfo.iso}</span>
                    </div>
                  </div>
                ) : (
                  <div className="text-red-400">{ulidInfo.error}</div>
                )}
              </div>
            </div>
          </div>
        ...
        {tab === "cron" && (
          <div className="w-full grid grid-cols-1 gap-4">
            <div>
              <h3 className="text-[#a78bfa] font-semibold mb-2">Cron Expression</h3>
              <input
                className="w-full rounded-xl px-4 py-2 bg-[#23243a]/70 border border-[#a78bfa66] text-gray-200"
                value={cron}
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {presets.map((p) => (
                  <button
                    key={p.expr}
                    className="px-3 py-1.5 rounded-xl border border-[#a78bfa66] text-gray-200 bg-[#23243a]/60 text-sm"
            ...
            <div className="rounded-xl border border-[#a78bfa99] bg-[#23243a]/80 p-3">
              {cronDesc.ok ? (
                <div className="text-gray-100">{cronDesc.text}</div>
              ) : (
                <div className="text-red-400">{cronDesc.error}</div>
              )}
            </div>
          </div>
        )}

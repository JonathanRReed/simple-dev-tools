...
export default function SchemaStudio() {
  ...
  const jsDoc = useCallback((schema: JSONSchema): string => {
    ...
    const lines: string[] = [];
    if (typeof schema.title === "string" && schema.title.trim()) lines.push(schema.title.trim());
    if (typeof schema.description === "string" && schema.description.trim()) {
      if (lines.length) lines.push("");
      lines.push(...schema.description.trim().split("\n"));
    }
  ...
  // components.schemas and Swagger 2.0 definitions, plus any $ref targets we can
  // resolve locally that weren't already named.
  const collectNamedDefs = useCallback(
    (root: JSONSchema): Record<string, any> => {
      ...
      // Ensure every referenced name resolves to a declaration.
    ...
    [sanitizeName, refName, collectRefs, resolveRef]
  );
    ...
    <ToolPage contentClassName="mx-auto max-w-6xl">
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-1.5">
          <h1 className="font-display text-3xl font-bold tracking-tight text-foreground">Schema &amp; Types Studio</h1>
